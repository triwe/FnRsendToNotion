...
- **Capture Feedback and Requests:** Seamlessly capture feedback and requests from users on the OnlyFans platform and send them to the appropriate Notion database. The function supports different data structures depending on whether the submission is feedback or a request.
...
```json
{
  ...
  "data": {
    ...
    "category": "Positive",
    "tags": "tag1, tag2",
    "memberProfileURL": "https://onlyfans.com/u123456789",
    "type": "feedback"
  }
}
```
...
```json
{
  ...
  "data": {
    ...
    "memberProfileURL": "https://onlyfans.com/u123456789",
    "type": "request",
    "requestDetails": "Detailed request description",
    "notes": "Manager notes",
    "budget": "500"
  }
}
...
To monitor the logs of your function, use the following command:
...
- **CORS Errors:** Ensure that the `Access-Control-Allow-Origin` header in the function is correctly set to `https://onlyfans.com`.
- **API Key Mismatch:** Double-check that the API keys provided in the request headers match the environment variables set in Google Cloud.
- **Organization Policy Restrictions:** If your deployment fails due to organization policy restrictions, you may need to modify the policy to allow necessary permissions.
...
3. **Re-deploy the Function:**
   - After modifying the policy, try deploying the function again using the deployment command provided in the Deploying the Function section.
...
You can test the function locally using tools like Postman or cURL. Here are examples of the expected request bodies for both feedback and requests:

**cURL Command for Feedback:**

```bash
curl -X POST https://us-central1-ofchat-ext-feedback.cloudfunctions.net/sendToNotion \
...
We welcome contributions! If you’d like to contribute to this project, please fork the repository and use a feature branch. Pull requests are warmly welcome. When contributing, please ensure any new features or changes are well-documented in the README.
